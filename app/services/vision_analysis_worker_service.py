"""Execute the existing Vision provider for orchestrator-assigned Business Assets."""
...
class VisionAnalysisWorkerService:
    def __init__(self, *, worker_instance_id: str, jobs=None, intelligence=None,
                 adapter=None, workflow=None) -> None:
        self.worker_instance_id = worker_instance_id
        self.jobs = jobs or VisionAnalysisJobRepository()
        self.intelligence = intelligence or AssetIntelligenceRepository()
        self.adapter = adapter or GptVisionAssetIntelligenceAdapter()
    ...
    def process_one(self) -> dict:
        ...
        self.workflow.report_started(job.asset_id, "VISION")
        ...
        if previous is not None:
            decision = self.workflow.report_completion(job.asset_id, "VISION", success=True)
            self._release_claim(job.asset_id)
            return {"processed": True, "asset_id": job.asset_id,
                    "status": decision.current_state.value, "reused": True}

        run_id = str(uuid5(NAMESPACE_URL, f"creator-os:vision-run:{job.asset_id}"))
        response = self.adapter.analyze(AssetIntelligenceProviderRequest(
            run_id=run_id, asset_id=job.asset_id,
            creator_profile_id=job.creator_profile_id, media_type=job.media_type,
            managed_media_path=job.file_path, original_filename=job.file_name,
            schema_version="vision_stage_v1",
        ))
        ...
        result = self.intelligence.save_provider_result(AssetIntelligenceProviderResult(
            result_id=str(uuid5(NAMESPACE_URL, f"creator-os:vision-result:{job.asset_id}:{job.attempt_number}")),
            ...
            metadata={"stage": "VISION", "attempt": job.attempt_number,
                      "started_at": response.started_at, "completed_at": response.completed_at,
                      "duration_ms": response.duration_ms},
        ))
        success = response.status == ProviderExecutionStatus.SUCCEEDED
        code = response.error_code.value if response.error_code else None
        decision = self.workflow.report_completion(
            job.asset_id, "VISION", success=success, error_code=code,
            error_message=response.error_message,
        )
        self._release_claim(job.asset_id)
        return {"processed": True, "asset_id": job.asset_id,
                "status": decision.current_state.value, "result_id": result.result_id,
