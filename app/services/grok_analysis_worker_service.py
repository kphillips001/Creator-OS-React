"""Execute the existing Grok Vision provider for orchestrator-assigned assets."""
...
class GrokAnalysisWorkerService:
    ...
    def process_one(self) -> dict:
        ...
        self.workflow.report_started(job.asset_id, "GROK")
        ...
        if previous is not None:
            decision = self.workflow.report_completion(job.asset_id, "GROK", success=True)
        ...
        run_id = str(uuid5(NAMESPACE_URL, f"creator-os:grok-run:{job.asset_id}"))
        response = self.adapter.analyze(AssetIntelligenceProviderRequest(
            ...
            schema_version="grok_semantic_stage_v1",
        ))
        ...
        result = self.intelligence.save_provider_result(AssetIntelligenceProviderResult(
            result_id=str(uuid5(NAMESPACE_URL, f"creator-os:grok-result:{job.asset_id}:{job.attempt_number}")),
            ...
            metadata={"stage": "GROK", "purpose": "semantic_interpretation",
                      "attempt": job.attempt_number, "started_at": response.started_at,
        ...
        decision = self.workflow.report_completion(
            job.asset_id, "GROK", success=success,
            error_code=response.error_code.value if response.error_code else None,
            error_message=response.error_message,
        )
        self._release_claim(job.asset_id)
