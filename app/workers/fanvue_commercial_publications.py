"""Feature-gated resumable Fanvue Commercial Publication worker."""
import logging
import time
from app.services.fanvue_commercial_publication_worker_service import FanvueCommercialPublicationWorkerService
...
def run(*, worker=None, heartbeat=None, max_cycles=None, poll_seconds=10):
    logger = logging.getLogger("fanvue-commercial-publications-worker")
    heartbeat = heartbeat or WorkerHeartbeatService(
        worker_name="Fanvue Commercial Publications", worker_type="publishing_worker",
        poll_interval_seconds=poll_seconds)
    worker = worker or FanvueCommercialPublicationWorkerService()
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.exception("Fanvue publication cycle failed")
