"""Supervised durable Photoshoot analysis orchestrator worker."""
...
def run(*, worker=None, heartbeat=None, max_cycles=None, poll_seconds=5):
    logger = logging.getLogger("photoshoot-analysis-worker")
    heartbeat = heartbeat or WorkerHeartbeatService(
        worker_name="Photoshoot Analysis", worker_type="analysis_worker",
        poll_interval_seconds=poll_seconds)
    worker = worker or PhotoshootAnalysisOrchestratorService(worker_instance_id=heartbeat.worker_instance_id)
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.exception("Photoshoot analysis cycle failed")
