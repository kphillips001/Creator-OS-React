"""Standalone supervised Vision Business Asset worker."""
...
def run(*, worker=None, heartbeat=None, max_cycles: int | None = None,
    ...
    logger = logging.getLogger("vision-analysis-worker")
    heartbeat = heartbeat or WorkerHeartbeatService(
        worker_name="Vision Analysis", worker_type="analysis_worker",
        poll_interval_seconds=poll_seconds,
    )
    worker = worker or VisionAnalysisWorkerService(worker_instance_id=heartbeat.worker_instance_id)
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    logger.exception("Vision failure-state persistence also failed")
                record_heartbeat_safely(logger, "failure", lambda: heartbeat.record_failure(error))
                logger.exception("Vision analysis cycle failed for asset_id=%s", failed_asset_id)
