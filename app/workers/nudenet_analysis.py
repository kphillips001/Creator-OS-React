"""Standalone durable NudeNet Business Asset worker."""
...
def run(*, worker=None, heartbeat=None, max_cycles: int | None = None, poll_seconds: int = 5) -> None:
    logger = logging.getLogger("nudenet-analysis-worker")
    heartbeat = heartbeat or WorkerHeartbeatService(
        worker_name="NudeNet Analysis", worker_type="analysis_worker", poll_interval_seconds=poll_seconds,
    )
    worker = worker or NudeNetAnalysisWorkerService(worker_instance_id=heartbeat.worker_instance_id)
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    logger.exception("NudeNet failure-state persistence also failed")
                record_heartbeat_safely(logger, "failure", lambda: heartbeat.record_failure(error))
                logger.exception("NudeNet analysis cycle failed for asset_id=%s", failed_asset_id)
