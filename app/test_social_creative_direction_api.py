...
from app.api import social_creative_direction as api
...
def test_missing_document_loads_exact_seed_defaults(monkeypatch):
    ...
    response = _client().get(
        "/api/v1/creator/social-creative-direction"
    )

    assert response.status_code == 200
    body = response.json()
    assert body["id"] is None
    assert body["creator_profile_id"] == 2
    assert body["purpose"] == api.DEFAULT_DOCUMENT["purpose"]
    assert body["wardrobe"] == api.DEFAULT_DOCUMENT["wardrobe"]
    assert repository.documents == {}


def test_document_saves_and_reloads_for_the_active_account(monkeypatch):
    repository = MemoryRepository()
    monkeypatch.setattr(api, "_account_id", lambda: 2)
    monkeypatch.setattr(
        api, "get_active_creator_profile",
    ...
    monkeypatch.setattr(api, "_repository", lambda: repository)
    payload = {**api.DEFAULT_DOCUMENT, "purpose": "Edited Ava purpose"}
    ...
    assert loaded.json()["purpose"] == "Edited Ava purpose"
    assert repository.save_calls[0][:2] == (2, "2")


def test_amanda_and_ava_documents_remain_independent(monkeypatch):
    repository = MemoryRepository()
    account = {"id": 2}
    profiles = {
    ...
    client.put(
        "/api/v1/creator/social-creative-direction",
        json={**api.DEFAULT_DOCUMENT, "purpose": "Ava direction"},
    )
    account["id"] = 3
    client.put(
        "/api/v1/creator/social-creative-direction",
        json={**api.DEFAULT_DOCUMENT, "purpose": "Amanda direction"},
    )

    assert repository.documents[(2, "2")]["purpose"] == "Ava direction"
    assert repository.documents[(1, "3")]["purpose"] == "Amanda direction"
