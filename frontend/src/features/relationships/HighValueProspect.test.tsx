...
it("renders compact ordered HVP badges and removes them only after confirmation",async()=>{
 ...
 fireEvent.click(await screen.findByRole("button",{name:"View Customer Intelligence"}));
 const card=within(screen.getByRole("dialog",{name:"Customer Intelligence"})).getByRole("region",{name:"Customer Intelligence"});
 ...
 fireEvent.click(screen.getByRole("button",{name:"Close Customer Intelligence details"}));
 ...
 expect(await within(screen.getByRole("dialog",{name:"Customer Intelligence"})).findByText("Prioritized")).toBeInTheDocument();
 fireEvent.click(screen.getByRole("button",{name:"Close Customer Intelligence details"}));
 ...
 fireEvent.click(screen.getByRole("button",{name:"More conversation controls"}));toggle=screen.getByRole("menuitem",{name:"High Value Prospect On"});fireEvent.click(toggle);
 const confirmation=screen.getByRole("dialog",{name:"Remove HVP?"});
 expect(within(confirmation).getByText(/HIGH market tier will not change/)).toBeInTheDocument();
 expect(within(row).getByText("HIGH")).toBeInTheDocument();
 fireEvent.click(within(confirmation).getByRole("button",{name:"Remove HVP"}));
 await waitFor(()=>expect(within(row).queryByTitle("High Value Prospect")).not.toBeInTheDocument());
