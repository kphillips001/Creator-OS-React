...
it("takes over, sends a manual message, and returns control without losing list state",async()=>{
 ...
 expect((await screen.findAllByText("MANUAL TAKEOVER")).length).toBeGreaterThanOrEqual(2);
 expect(screen.getByRole("button",{name:"RETURN TO AVA"})).toBeInTheDocument();
 expect(screen.getByPlaceholderText("Type a message as Ava...")).toBeInTheDocument();
 ...
 fireEvent.click(screen.getByRole("button",{name:"Return to Ava Auto"}));
 fireEvent.click(screen.getByRole("dialog").querySelector("button:last-child")!);
