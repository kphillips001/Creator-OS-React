...
export function RegenerationStudioPage() {
  ...
  const active = Boolean(workspace && ["QUEUED", "RUNNING", "WAITING_EXTERNAL"].includes(workspace.operation.status));
  ...
  const applySource = useCallback((nextSourceId: string) => {
    ...
    setSelected(new Set()); setCount(1); setError(""); setMessage(""); setResetIntent(null);
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    const response = await fetch(`/api/v1/regeneration/${encodeURIComponent(operationId)}`);
    ...
    setWorkspace(body); setCount(body.run.requestedCount);
    return body;
  }, [operationId]);
  ...
  useEffect(() => {
    if (!operationId) return;
    let mounted = true;
    const read = () => loadWorkspace().catch((reason) => { if (mounted) setError(reason instanceof Error ? reason.message : "Unable to load regeneration."); });
    void read();
    const timer = window.setInterval(() => { if (!workspace || ["QUEUED", "RUNNING", "WAITING_EXTERNAL"].includes(workspace.operation.status)) void read(); }, 1500);
    return () => { mounted = false; window.clearInterval(timer); };
  }, [loadWorkspace, operationId, workspace?.operation.status]);

  const regenerate = async () => {
    if (!sourceId || !eligible || busy) return;
    setBusy(true); setError(""); setMessage("");
    try {
      const response = await fetch("/api/v1/regeneration", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source_generated_image_id: sourceId, count }) });
  ...
  const requestReset = () => { if (active || busy) return; if (unresolved.length) setResetIntent({ kind: "reset" }); else applySource(""); };
  const confirmArchiveAndContinue = async () => {
    if (!resetIntent) return;
    const intent = resetIntent;
    if (await transitionSelected("archive", unresolved.map((item) => item.resultId))) applySource(intent.kind === "switch" ? intent.sourceId : "");
  };
  ...
  const failed = Number(workspace?.operation.metadata.failedCount ?? workspace?.results.filter((item) => item.status === "FAILED").length ?? 0);
  return <section className="regeneration-studio">
    ...
    {source && eligible && !active && <div className="regeneration-controls"><label>Number of variations<select aria-label="Number of variations" value={count} onChange={(event) => setCount(Number(event.target.value))}>{[1,2,3,4,5].map((value) => <option value={value} key={value}>{value}</option>)}</select></label><button type="button" disabled={busy} onClick={regenerate}><RotateCw size={17} /> {busy ? "Starting…" : "Regenerate"}</button></div>}
    ...
    {workspace && visibleResults.length > 0 && <div className="regeneration-results"><div className="regeneration-results__header"><div><h2>Variations</h2><span>Select successful results to send to Generation Library or Archive.</span></div>{selectable.length > 1 && <button type="button" onClick={() => setSelected(selected.size === selectable.length ? new Set() : new Set(selectable.map((item) => item.resultId)))}>{selected.size === selectable.length ? "Clear selection" : "Select all"}</button>}</div><div className="regeneration-grid">{visibleResults.map((item: RegenerationResult) => <article className={`regeneration-card regeneration-card--${item.status.toLowerCase()}`} key={item.resultId}>{item.mediaUrl ? <button className="regeneration-card__image" type="button" onClick={() => setPreview(item.mediaUrl)}><img src={item.mediaUrl} alt={`Regenerated variation ${item.variationIndex}`} /></button> : <div className="regeneration-card__placeholder"><ImageOff /></div>}<div><span>Variation {item.variationIndex}</span><strong>{item.disposition === "PROMOTED" ? "Sent to Generation Library" : item.status}</strong>{item.status === "SUCCEEDED" && item.disposition === "PENDING_REVIEW" && <label><input type="checkbox" checked={selected.has(item.resultId)} onChange={() => toggle(item.resultId)} /> Select</label>}{item.disposition === "PROMOTED" && <Check aria-label="Promoted" />}{item.errorMessage && <small>{item.errorMessage}</small>}</div></article>)}</div>{selected.size > 0 && <div className="regeneration-promote"><button type="button" disabled={busy} onClick={() => void transitionSelected("archive")}><Archive size={17} /> Archive Selected</button><button type="button" disabled={busy} onClick={() => void transitionSelected("promote")}><ExternalLink size={17} /> Send Selected to Generation Library</button></div>}</div>}
