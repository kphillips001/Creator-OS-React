...
export function DeveloperNotesPage() {
  ...
  const [deleting,setDeleting]=useState<DeveloperTodo|null>(null);
  ...
  const remove=async(todo:DeveloperTodo)=>{setSaving(todo.id);setError("");try{await developerNotesApi.delete(todo.id);setTodos(items=>items.filter(item=>item.id!==todo.id));setDeleting(null);if(editingNote===todo.id)setEditingNote(null);}catch(reason){setError(reason instanceof Error?reason.message:"Unable to delete Developer TODO.");setDeleting(null);}finally{setSaving(null);}};
  ...
  return <section className="developer-notes-page"><PageHeader title="Developer Notes" description="Keep track of future Creator_OS development work." />
    <section className="developer-todos" aria-labelledby="developer-todos-title"><header><div><h2 id="developer-todos-title">TODO</h2><p>A simple persistent development checklist.</p></div><button onClick={()=>setCreating(true)} type="button">+ New TODO</button></header>
      ...
      {loading?<p className="developer-todos__loading">Loading TODOs…</p>:<ul>{todos.map(todo=><li className={todo.completed?"developer-todo developer-todo--completed":"developer-todo"} key={todo.id}>
        <div className="developer-todo__main"><label><input aria-label={`Mark ${todo.title} ${todo.completed?"open":"complete"}`} checked={todo.completed} disabled={saving===todo.id} onChange={()=>void toggle(todo)} type="checkbox"/><span><strong>{todo.title}</strong><small>Added {localDate(todo.createdAt)}</small></span></label><div className="developer-todo__end">{todo.completed&&todo.completedAt&&<span className="developer-todo__completed-date">Completed {localDate(todo.completedAt)}</span>}<button aria-label={`Delete ${todo.title}`} className="developer-todo__delete" disabled={saving===todo.id} onClick={()=>setDeleting(todo)} title="Delete TODO" type="button"><Trash2 size={16}/></button></div></div>
    ...
    {deleting&&<div className="developer-todo-confirm" role="presentation"><div aria-labelledby="delete-todo-title" aria-modal="true" className="developer-todo-confirm__panel" role="dialog"><h2 id="delete-todo-title">Delete TODO?</h2><p>Are you sure you want to permanently delete this TODO?</p><strong>{deleting.title}</strong><div><button disabled={saving===deleting.id} onClick={()=>setDeleting(null)} type="button">Cancel</button><button className="developer-todo-confirm__delete" disabled={saving===deleting.id} onClick={()=>void remove(deleting)} type="button">{saving===deleting.id?"Deleting…":"Delete"}</button></div></div></div>}
  </section>;
}
