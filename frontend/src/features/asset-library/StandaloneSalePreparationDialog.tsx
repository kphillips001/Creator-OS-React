...
export function StandaloneSalePreparationDialog({ asset, retry = false, onClose, onStarted }: {
  asset: AssetLibraryItem; retry?: boolean; onClose: () => void;
  ...
  const [price, setPrice] = useState(((initial?.priceMinor || 1000) / 100).toFixed(2));
  const [destination, setDestination] = useState<StandaloneSaleDestination>(() => initialDestination(initial));
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/assets/${asset.assetId}/sale-preparation${retry ? "/retry" : ""}`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
          priceMinor, destinations: [destination],
  ...
  return <div className="sale-preparation-dialog" role="dialog" aria-modal="true" aria-labelledby="prepare-single-image-title"><div>
    ...
    <fieldset className="single-image-sale-destinations"><legend>Selling / Publishing</legend>
      <label className={chatSelected ? "is-selected" : ""}><input aria-label="Chat Selling" type="radio" name="standalone-sale-destination" checked={chatSelected} onChange={() => setDestination("CHAT")} /><span className="destination-check" aria-hidden="true">{chatSelected && <Check />}</span><span><strong>Chat Selling</strong><small>Selective teaser for 1:1 sales</small></span></label>
      <label className={vaultSelected ? "is-selected" : ""}><input aria-label="Ava's Content Vault" type="radio" name="standalone-sale-destination" checked={vaultSelected} onChange={() => setDestination("CONTENT_VAULT")} /><span className="destination-check" aria-hidden="true">{vaultSelected && <Check />}</span><span><strong>Ava&apos;s Content Vault</strong><small>Full-blur teaser for Vault posts</small></span></label>
    </fieldset>
    {chatSelected && <section className="single-image-teaser-choice"><div className="single-image-teaser-heading"><strong>Chat Teaser</strong><span>Selective Blur</span></div><p>Protect the reveal while keeping enough visible to sell.</p>{chatTeaserDraft?.previewUrl && <ContainedMediaImage alt="Chat selective blur teaser" src={`${chatTeaserDraft.previewUrl}?v=${Date.now()}`} />}{chatTeaserDraft && <em><Check /> Teaser Ready</em>}<button className="sale-preparation-secondary" type="button" onClick={() => setEditing("CHAT")}><Pencil />{chatTeaserDraft ? "Replace Selective Teaser" : "Edit Selective Teaser"}</button>{!chatTeaserDraft && <small>Save and accept a selective teaser before preparation.</small>}</section>}
    ...
    <footer className="single-image-sale-footer"><button className="sale-preparation-secondary" onClick={onClose} type="button">Cancel</button><button className="sale-preparation-primary" disabled={priceMinor == null || saving || (chatSelected && !chatTeaserDraft) || (vaultSelected && vaultStyle === "SELECTIVE_BLUR" && vaultTeaserDraft?.teaserStyle !== "SELECTIVE_BLUR")} onClick={() => void submit()} type="button">{saving ? "Starting..." : preparation?.foundationReady ? "Save Changes" : retry ? "Retry Preparation" : "Prepare for Sale"}</button></footer>
