...
describe("Business navigation", () => {
  it("presents only primary operator workflows and moves administration to Advanced", () => {
    ...
    const advanced = navigationGroups.find(
...
describe("Creator navigation", () => {
  it("moves creator configuration into Administration without duplication", () => {
    ...
    expect(
      administration?.items.slice(1, 5).map((item) => [item.label, item.path]),
    ).toEqual([
...
describe("Administration navigation", () => {
  it("appears before Tools and System", () => {
    ...
    expect(labels[index + 1]).toBe("Tools");
    expect(
...
describe("Tools navigation", () => {
  it("places both competitor intelligence workspaces under Tools", () => {
    const labels = navigationGroups.map((group) => group.label);
    const index = labels.indexOf("Tools");
    expect(labels[index - 1]).toBe("Administration");
    expect(labels[index + 1]).toBe("System");
    expect(
      navigationGroups[index]?.items.map((item) => [item.label, item.path]),
    ).toEqual([
      ["AI Developer Notes", "/tools/ai-training"],
      ["X Competitor Intelligence", "/tools/x-intelligence"],
      ["IG Competitor Intelligence", "/tools/ig-intelligence"],
    ]);
  });
});
...
describe("Developer Tools navigation", () => {
  it("includes the read-only Customer Commerce workspace", () => {
    ...
    expect(
      developer?.items.map((item) => [item.label, item.path]),
    ).toContainEqual(["Customer Commerce", "/developer/customer-commerce"]);
    expect(
      developer?.items.map((item) => [item.label, item.path]),
    ...
    expect(
      ...
      "/developer/customer-sales-brain",
    ]);
    expect(
      developer?.items.map((item) => [item.label, item.path]),
    ).toContainEqual([
      "Commercial Offering Selector",
      "/developer/offering-selector",
    ]);
  });
});
...
describe("AI navigation", () => {
  ...
  it("includes live AI Training controls separately from developer notes", () => {
    const ai = navigationGroups.find((group) => group.label === "AI");
    expect(ai?.items.map((item) => [item.label, item.path])).toContainEqual([
      "AI Training", "/agents/ai-training",
    ]);
  });
});
